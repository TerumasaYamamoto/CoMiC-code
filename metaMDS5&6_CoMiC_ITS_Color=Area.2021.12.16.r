...
class.ASV <- class.table[,1:(ncol(class.table)-7)] # I have changed 7 into 6 because the table did not contain the Species column.
class.taxonomy <- class.table[,(ncol(class.table)-6):ncol(class.table)] # Also, I have changed 6 into 5.
...
# nmds
percent.t <- t (percent)
nmds <-metaMDS(percent.t, trace=F, distance="bray", perm=10000000,k=6)
env <- DESIGN [,2:3]
...
ggplot()+
geom_point(data=data, aes(x=MDS5,y=MDS6, color = Farm, shape=Type),size=4)+
scale_colour_manual(values = c("#141433","#3d3d99","#3333ff","#6666ff","#ccccff","#660000","#cc0000","#ff0000","#ff6666","#ffcccc","#296629","#3d993d","#33ff33","#99ff99","#ccffcc"))+  # if you want to change the colors
...
# Save
ggsave(file = "NMDS_5&6_Color=Area.png",width =7, height =6.5)